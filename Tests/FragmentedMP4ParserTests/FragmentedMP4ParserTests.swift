...
class FragmentedMP4ParserTests: XCTestCase {
  ...
  func testSuccessfulHEVCParsing() {
    let path = FragmentedMP4ParserTests.resourcePath(
      withFilename: "sample_video_fragmented_hevc.mp4")
    let parser = FragmentedMP4Parser(path: path)
  ...
  func testFailedParsing() {
    let path = FragmentedMP4ParserTests.resourcePath(
      withFilename: "sample_video_non_fragmented.mp4")
    let parser = FragmentedMP4Parser(path: path)
  ...
  func testMediaInfo() {
    ...
    XCTAssertEqual(mediaInfo.audioCodec, "mp4a")
